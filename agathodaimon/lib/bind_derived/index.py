...
def seed_bytes(identity: str, pin: str) -> bytes:
    if len(identity) != 64 or any(char not in "0123456789abcdef" for char in identity):
        raise ValueError("agathodaimon-staff-identity-invalid")
...
def verify_derived(pin: str, expected_public_key: str | None = None) -> dict[str, Any]:
    ...
    if not expected:
        return {
            ...
            "posture": "UNBOUND",
            "verified": False,
            "firstMissingSignal": "agathodaimon-derived-unbound",
        }
    try:
    ...
    try:
        ...
        return {
            ...
            "posture": "DERIVED_BOUND",
            "verified": verified,
            "firstMissingSignal": "none" if verified else "agathodaimon-staff-derived-key-mismatch",
        }
    finally:
        candidate.close()
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="agathodaimon-sacred-credential")
