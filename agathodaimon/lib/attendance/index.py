...
def _context(purpose: str, raw: object) -> dict[str, Any]:
    ...
    fields = {"session.prove": ("ticket", "read", "method", "target"), "session.clear": ("ticket",), "capability.mint": ("ticket", "action", "target", "profile"), "pin.change": ("ticket", "action", "target")}[purpose]
...
class AttendanceStaff:
    ...
    def signing_status(self) -> dict[str, object]:
        ...
        return self._success(schema="caduceus.staff.signing.status.v1", posture="BOUND")

    def _success(self, **projection: object) -> dict[str, object]:
        self._bound("response.projection")
        if self._public is None or self._epoch is None:
            raise AccessRefused()
        return {"ok": True, "public_key": self._public.hex(), "epoch": self._epoch, **projection}
    ...
    def _token(self, claims: Mapping[str, Any]) -> str:
        ...
        return f"{_b64u(payload)}.{_b64u(signature)}"
    ...
    def challenge_mint(self, purpose: str, context: Mapping[str, Any]) -> dict[str, object]:
        ...
        return self._success(schema="caduceus.staff.attendance.challenge.v1", challenge_id=cid, challenge=challenge, expires_at=expires)
    ...
    def session_mint(self, *, pin: str, challenge_id: str, signature: str) -> dict[str, object]:
        ...
        claims = {"kind":"attendance","version":1,"attendance_id":candidate.jti,"document_key_thumbprint":thumbprint,"epoch":self._epoch,"issued_at":int(self._clock()),"jti":candidate.jti}
        ticket = self._token(claims); candidate = _Attendance(ticket, thumbprint, key, self._epoch or "", candidate.jti)
        with self._lock: self._room(self._attendances); self._attendances[ticket] = candidate; self._attendance_ids[candidate.jti] = candidate
        self._audit("session.mint", "OK", attendance=ticket, jti=candidate.jti)
        return self._success(schema="caduceus.staff.attendance.session-mint.v1", ticket=ticket, document_key_thumbprint=thumbprint, attendance_id=candidate.jti)

    def session_prove(self, *, ticket: str, challenge_id: str, signature: str, read: str, method: str, target: str) -> dict[str, object]:
        item = self._consume("session.prove", challenge_id, {"ticket":ticket,"read":read,"method":method,"target":target}); attendance = self._active(ticket); self._verify(attendance, challenge_id, item, signature); self._audit("session.prove", "OK", attendance=ticket); return self._success()

    def capability_mint(self, *, ticket: str, challenge_id: str, signature: str, action: str, target: str, profile: str) -> dict[str, object]:
        item = self._consume("capability.mint", challenge_id, {"ticket":ticket,"action":action,"target":target,"profile":profile}); attendance = self._active(ticket); self._verify(attendance, challenge_id, item, signature)
        jti, expires = self._token_factory(), int(self._clock()) + CAPABILITY_TTL_SECONDS; claims = {"kind":"capability","version":1,"id":jti,"jti":jti,"parent_attendance_id":attendance.jti,"action":action,"target":target,"profile":profile,"epoch":self._epoch,"exp":expires}; token = self._token(claims)
        with self._lock: self._room(self._capabilities); self._capabilities[token] = _Capability(token,ticket,action,target,profile,expires,jti)
        self._audit("capability.mint", "OK", attendance=ticket, jti=jti, scope=f"{action}:{target}"); return self._success(capability=token, expires_at=expires)
    ...
    def session_clear(self, *, ticket: str, challenge_id: str, signature: str) -> dict[str, object]:
        ...
        with self._lock: self._attendances.pop(ticket, None); self._attendance_ids.pop(attendance.jti, None); self._capabilities = {k:v for k,v in self._capabilities.items() if v.parent_ticket != ticket}
        self._audit("session.clear", "OK", attendance=ticket); return self._success()
    ...
    def pin_change(self, *, ticket: str, challenge_id: str, signature: str, capability: str, old_pin: str, new_pin: str) -> dict[str, object]:
        ...
        self._audit("pin.change","OK",jti=attendance.jti); return self._success()

    def attendance_current(self, attendance_id: str) -> dict[str, object]:
        self._bound("attendance.current")
        key = _bounded_text(attendance_id, maximum=512)
        with self._lock: current = key in self._attendance_ids
        self._audit("attendance.current", "OK" if current else "INACTIVE", jti=key)
        return self._success(attendance_id=key, current=current)
    ...
    def dispatch(self, request: Mapping[str, Any]) -> dict[str, object]:
        ...
        if op == "capability.mint": return self.capability_mint(ticket=request.get("ticket"),challenge_id=request.get("challenge_id"),signature=request.get("signature"),action=request.get("action"),target=request.get("target"),profile=request.get("profile"))
        ...
        if op == "attendance.current": return self.attendance_current(_bounded_text(request.get("attendance_id"), maximum=512))
        raise AccessRefused()


class StaffSocketDaemon:
    def __init__(self, staff: AttendanceStaff, path: str = "/run/caduceus/agathodaimon-staff.sock") -> None: self.staff, self.path, self._stop = staff, path, threading.Event()
