...
def _receipt(*, ok: bool, changed: bool, signal: str, **fields: Any) -> dict[str, Any]:
    return {
        ...
        "writer": "agathodaimon.lib.source_map",
...
def reseed() -> dict[str, Any]:
    ...
    try:
        ...
        return _receipt(ok=False, changed=False, signal=f"agathodaimon-source-map-reseed-{exc}")
    ...
    try:
        ...
        return _receipt(ok=False, changed=False, signal=f"agathodaimon-source-map-reseed-write-failed: {exc}", **common)
    finally:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="agathodaimon-profile-sources-reseed")
    parser.add_argument("command", choices=["reseed"])
    args = parser.parse_args(argv)
    value = reseed() if args.command == "reseed" else _receipt(ok=False, changed=False, signal="agathodaimon-source-map-command-invalid")
