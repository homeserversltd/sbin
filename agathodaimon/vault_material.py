...
def open_from_seated_record(payload: object) -> dict[str, Any]:
    if not isinstance(payload, dict) or set(payload) != {"device", "mapper"}:
        return _receipt(False, True, "caduceus-vault-open-request-invalid")
    ...
    try:
        ...
        return _receipt(
            result.returncode == 0,
            True,
            "none" if result.returncode == 0 else "caduceus-vault-keyman-open-refused",
        )
    except (OSError, subprocess.SubprocessError, sacred_credential.CaduceusAccessRefused):
        return _receipt(False, True, "caduceus-vault-keyman-open-refused")
    finally:
        _wipe(material)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        value = _receipt(False, True, "caduceus-vault-open-request-invalid")
