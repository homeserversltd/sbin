"""Receipt-backed date and time settings."""
from __future__ import annotations
try:
    from ._common import build_settings_widget
except ImportError:
    import importlib.util as _importlib_util, sys as _sys
    _spec = _importlib_util.spec_from_file_location("_common", __file__.replace(__file__.split("/")[-1], "_common.py"))
    _module = _importlib_util.module_from_spec(_spec); _sys.modules["_common"] = _module; _spec.loader.exec_module(_module)
    from _common import build_settings_widget
PLUG={"id":"date-time","title":"Date & Time","icon":"preferences-system-time-symbolic","order":100,"parent":None}
SPECS=[
 {"field":"timezone","title":"Time zone"},
 {"field":"ntp_enabled","title":"Automatic date and time","kind":"switch"},
 {"field":"automatic_timezone","title":"Automatic time zone","kind":"switch"},
 {"field":"date_format","title":"Date format"},
 {"field":"time_format","title":"Time format"},
]
def build_widget(): return build_settings_widget(PLUG,"datetime","System clock",SPECS)
