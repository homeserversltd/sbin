...
def _user_dirs(user: str) -> tuple[pwd.struct_passwd, dict[str, Path]]:
    try:
        ...
        raise DesktopCacheError("agathodaimon-desktop-cache-user-missing") from error

    owns_environment = os.geteuid() == account.pw_uid
    home_value = os.environ.get("HOME") if owns_environment else None
    home = _absolute(home_value or account.pw_dir, "agathodaimon-desktop-cache-home-invalid")
    ...
    state = xdg("XDG_STATE_HOME", home / ".local/state", "agathodaimon-desktop-cache-state-invalid")
    cache = xdg("XDG_CACHE_HOME", home / ".cache", "agathodaimon-desktop-cache-cache-invalid")
    runtime = xdg("XDG_RUNTIME_DIR", Path(f"/run/user/{account.pw_uid}"), "agathodaimon-desktop-cache-runtime-invalid")
...
def _run_as_user(
    ...
    if os.geteuid() not in (0, account.pw_uid):
        raise DesktopCacheError("agathodaimon-desktop-cache-user-switch-refused")
...
def _command_domain(
    ...
    if required_path is not None and not required_path.is_dir():
        ...
        return _result(
            ...
            optional,
            "failed",
            blocker=f"agathodaimon-desktop-cache-{domain}-input-missing",
    ...
    if utility is None:
        ...
        return _result(
            ...
            optional,
            "failed",
            blocker=f"agathodaimon-desktop-cache-{domain}-utility-missing",
    ...
    if result.returncode != 0:
        return _result(
            ...
            optional,
            "failed",
            blocker=f"agathodaimon-desktop-cache-{domain}-refresh-failed",
            **fields,
        )
...
def _bounded_deletion(path: Path, roots: Sequence[Path]) -> Path:
    ...
    for root in roots:
        ...
        if relative != Path("."):
            return candidate
    raise DesktopCacheError("agathodaimon-desktop-cache-delete-out-of-root")
...
def _wofi_domain(optional: bool, directories: dict[str, Path]) -> dict[str, Any]:
    ...
    try:
        ...
        if bounded.is_dir():
            raise DesktopCacheError("agathodaimon-desktop-cache-delete-not-file")
        cache_file.unlink()
...
def refresh(user: str, declarations: Sequence[tuple[str, bool]]) -> dict[str, Any]:
    ...
    for domain, optional in declarations:
        if domain in seen:
            raise DesktopCacheError("agathodaimon-desktop-cache-domain-duplicate")
        seen.add(domain)
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="agathodaimon-desktop-cache")
