...
def validated_timezone(value: Any) -> str:
    if not isinstance(value, str) or not ZONE_RE.fullmatch(value):
        raise ValueError("agathodaimon-household-time-timezone-invalid")
    candidate = (zoneinfo_root() / value).resolve()
    try:
        candidate.relative_to(zoneinfo_root().resolve())
    except ValueError as exc:
        raise ValueError("agathodaimon-household-time-timezone-invalid") from exc
    if not candidate.is_file():
        raise ValueError("agathodaimon-household-time-timezone-not-installed")
    return value
...
def read_state() -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError("agathodaimon-household-time-state-invalid") from exc
    if not isinstance(value, dict) or value.get("schema_version") != 1:
        raise ValueError("agathodaimon-household-time-state-invalid")
    return value
...
def resolve() -> dict[str, Any]:
    ...
    request = Request(
        ...
        headers={"Accept": "application/json", "User-Agent": "agathodaimon-household-time/1"},
    )
    try:
        ...
        return receipt(
            ...
            first_missing_signal="agathodaimon-household-time-provider-unavailable",
...
def _hold_seconds() -> int:
    ...
    try:
        ...
        raise ValueError("agathodaimon-household-time-hold-invalid") from exc
    if not 60 <= value <= MAX_HOLD_SECONDS:
        raise ValueError("agathodaimon-household-time-hold-invalid")
    return value
...
def ensure_ntp() -> dict[str, Any]:
    ...
    if selected is None:
        return receipt("ensure-ntp", ok=False, changed=False,
                       first_missing_signal="agathodaimon-household-time-ntp-provider-missing", providers=[name for name, _, _ in candidates])
    ...
    return receipt(
        ...
        post_apply=readback, first_missing_signal="none" if synchronized else "agathodaimon-household-time-ntp-not-synchronized",
    )
...
def set_timezone(candidate: str) -> dict[str, Any]:
    ...
    if result.returncode != 0 or applied != requested:
        return receipt("set-timezone", ok=False, changed=False, requested_timezone=requested,
                       ...
                       first_missing_signal="agathodaimon-household-time-timezone-readback-failed")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="agathodaimon-household-time")
