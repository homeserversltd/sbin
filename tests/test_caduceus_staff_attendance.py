...
_staff_spec = _importlib_util.spec_from_file_location("agathodaimon.python.staff_daemon_face", ROOT / "agathodaimon/python/staff-daemon/index.py")
...
class FakeKeyman:
    def __init__(self, pin="2468", epoch="a" * 64):
    ...
    def change_caduceus_pin(self, old, new):
        ...
        self.pin, self.epoch, self.signer = new, f"{int(self.epoch, 16) + 1:064x}", Signer(f"{int(self.epoch, 16) + 1:064x}")
...
class AttendanceTests(unittest.TestCase):
    ...
    def cap(self,ticket, action="network.apply",target="lan",profile="caduceus"):
        ch=self.challenge("capability.mint",{"ticket":ticket,"action":action,"target":target,"profile":profile})
        return self.staff.capability_mint(ticket=ticket,challenge_id=ch["challenge_id"],signature=self.sign(self.key,ch),action=action,target=target,profile=profile)
    def refused(self, call):
        with self.assertRaises(AccessRefused): call()
    def test_admitted_surface_binds_public_epoch_and_dot_signed_ticket(self):
        status=self.staff.signing_status(); self.assertTrue(status["ok"]); self.assertEqual(status["posture"],"BOUND"); self.assertEqual(status["epoch"],"a" * 64); self.assertEqual(status["public_key"],self.keyman.signer.public_key_hex)
        ticket=self.mint()["ticket"]; payload64, signature64 = ticket.split("."); self.assertNotIn("=",ticket); payload=base64.urlsafe_b64decode(payload64+"=="); signature=base64.urlsafe_b64decode(signature64+"==")
        self.keyman.signer.private.public_key().verify(signature,payload); claims=json.loads(payload); self.assertEqual(claims["kind"],"attendance"); self.assertEqual(claims["epoch"],"a" * 64)
    def test_capability_claims_profile_signature_and_mismatch(self):
        ticket=self.mint()["ticket"]; cap=self.cap(ticket,profile="home"); payload64, signature64=cap["capability"].split("."); payload=base64.urlsafe_b64decode(payload64+"=="); self.keyman.signer.private.public_key().verify(base64.urlsafe_b64decode(signature64+"=="),payload)
        claims=json.loads(payload); self.assertTrue({"kind","version","id","parent_attendance_id","action","target","profile","epoch","exp"}.issubset(claims)); self.assertEqual(claims["profile"],"home"); self.assertEqual(claims["parent_attendance_id"],self.staff._attendances[ticket].jti); self.assertEqual(claims["epoch"],"a" * 64)
        ch=self.challenge("capability.mint",{"ticket":ticket,"action":"network.apply","target":"lan","profile":"home"}); self.refused(lambda:self.staff.capability_mint(ticket=ticket,challenge_id=ch["challenge_id"],signature=self.sign(self.key,ch),action="network.apply",target="lan",profile="other"))
    def test_currentness_by_id_and_success_projections(self):
    ...
    def test_staff_daemon_is_seated_in_python_band(self):
        self.assertTrue((ROOT / "agathodaimon" / "python" / "staff-daemon" / "index.py").is_file())
    ...
    def test_wrong_old_pin_spends_capability_but_keeps_attendance_and_epoch(self):
        ticket=self.mint()["ticket"]; attendance_id=self.staff._attendances[ticket].jti; before=self.staff.signing_status().copy(); cap=self.cap(ticket,PIN_ROTATION_ACTION,PIN_ROTATION_TARGET); ch=self.challenge("pin.change",{"ticket":ticket,"action":PIN_ROTATION_ACTION,"target":PIN_ROTATION_TARGET})
        ...
        self.assertEqual(self.keyman.pin,"2468"); self.assertEqual(before,self.staff.signing_status()); self.assertTrue(self.staff.attendance_current(attendance_id)["current"]); self.assertNotIn(cap["capability"],self.staff._capabilities)
        ...
        self.assertEqual(result["epoch"],"a" * 63 + "b"); self.refused(lambda:self.prove(ticket)); self.assertFalse(AttendanceStaff(self.keyman).attendance_current(attendance_id)["current"])
    def test_production_sized_ticket_round_trips_every_ticket_context(self):
        keyman, clock = LongTokenKeyman(), Clock(); n = 0
        def token():
            nonlocal n; n += 1; return f"{n:02d}" + "x" * 204
        ...
        cap, signature=signed("capability.mint",{"ticket":ticket,"action":"network.apply","target":"lan","profile":"caduceus"})
        self.assertIn("capability",staff.capability_mint(ticket=ticket,challenge_id=cap["challenge_id"],signature=signature,action="network.apply",target="lan",profile="caduceus"))
