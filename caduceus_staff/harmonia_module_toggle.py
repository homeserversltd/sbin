...
def _reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, child in pairs:
        if key in value:
            raise ToggleError("caduceus-harmonia-module-toggle-config-duplicate-key")
        value[key] = child
    return value
...
def _load(raw: str) -> dict[str, Any]:
    try:
        ...
        raise ToggleError("caduceus-harmonia-module-toggle-config-invalid") from exc
    if not isinstance(value, dict):
        raise ToggleError("caduceus-harmonia-module-toggle-config-not-object")
    return value
...
def _object_members(raw: str, start: int = 0) -> tuple[int, int, dict[str, tuple[int, int]]]:
    ...
    if start >= len(raw) or raw[start] != "{":
        raise ToggleError("caduceus-harmonia-module-toggle-config-invalid")
    index = start + 1
...
def _updated_bytes(raw: str, module: str, state: str) -> tuple[str, list[str]]:
    ...
    if harmonia is not None and not isinstance(harmonia, dict):
        raise ToggleError("caduceus-harmonia-module-toggle-harmonia-not-object")
    ...
    if not isinstance(previous, list) or any(not isinstance(item, str) for item in previous):
        raise ToggleError("caduceus-harmonia-module-toggle-disabled-modules-invalid")
...
def list_disabled() -> dict[str, Any]:
    path = config_path()
    if not path.is_file():
        return _receipt(None, "list", [], None, ok=False, signal="caduceus-harmonia-module-toggle-config-missing")
    try:
...
def toggle(module: str, state: str) -> dict[str, Any]:
    if not MODULE_ID.fullmatch(module):
        return _receipt(module, state, [], None, ok=False, signal="caduceus-harmonia-module-toggle-module-invalid")
    path = config_path()
    if not path.is_file():
        return _receipt(module, state, [], None, ok=False, signal="caduceus-harmonia-module-toggle-config-missing")
    try:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="caduceus-harmonia-module-toggle")
