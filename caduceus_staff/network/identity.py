...
def device_list() -> list[dict[str, Any]]:
    ...
    for reservation in dhcp.reservations():
        item = records.setdefault(reservation["mac"], {
            "mac": reservation["mac"], "observed_lease": None, "declared_reservation": None,
            "dns_names": [], "mismatches": [],
        })
        item["declared_reservation"] = {"ip": reservation["ip"], "hostname": reservation["hostname"], "provenance": "declared"}
    for lease in dhcp.leases():
        item = records.setdefault(lease["mac"], {
            "mac": lease["mac"], "observed_lease": None, "declared_reservation": None,
            "dns_names": [], "mismatches": [],
        })
    ...
    for item in records.values():
        ...
        if declared and dns_by_ip.get(declared["ip"]) and not item["dns_names"]:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        return emit({
            "schema": "caduceus.staff.network.identity.v1",
            "actuator": "network.identity.device_list",
            "action": "device_list",
            "result": device_list(),
