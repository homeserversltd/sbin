...
def _read_config() -> tuple[str, list[list[str]]]:
    ...
    result = _run(command)
    _must(result, "caduceus-ssh-exposure-config-read-refused")
...
def _directive_values(raw: str) -> dict[str, str]:
    ...
    for line in raw.splitlines():
        ...
        if name in values:
            raise Refusal("caduceus-ssh-exposure-directive-ambiguous")
...
def _write_config(raw: str) -> list[list[str]]:
    ...
    _must(_run(command, input_text=raw), "caduceus-ssh-exposure-config-write-refused")
    return [command]
...
def _validate() -> list[list[str]]:
    ...
    if result.returncode != 0 and "Missing privilege separation directory: /run/sshd" in result.stderr:
        ...
        _must(_run(runtime), "caduceus-ssh-exposure-runtime-directory-refused")
        result = _run(command)
        commands.append(command)
    _must(result, "caduceus-ssh-exposure-config-invalid")
    return commands
...
def _service_unit() -> tuple[str, list[list[str]]]:
    ...
    raise Refusal("caduceus-ssh-exposure-service-missing")
...
def toggle(state: str) -> dict[str, Any]:
    ...
    try:
        ...
        for action in (("enable", "start") if state == "on" else ("disable", "stop")):
            ...
            commands.append(command)
            _must(_run(command), f"caduceus-ssh-exposure-service-{action}-refused")
        ...
        if service["active"] != expected_active or service["enabled"] != expected_active:
            raise Refusal("caduceus-ssh-exposure-service-readback-mismatch")
        if directives != desired:
            raise Refusal("caduceus-ssh-exposure-config-readback-mismatch")
        ...
        if config_written and original is not None:
            try:
                ...
                return _receipt(state, ok=False, changed=changed, unchanged=unchanged, readback=None, commands=commands, signal="caduceus-ssh-exposure-config-restore-refused")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="caduceus-ssh-exposure")
