...
def schedule(v):
 if not isinstance(v,dict): raise Refusal("caduceus-nas-schedule-invalid")
 if set(v)-{"frequency","time","weekday","day","at","enabled"}: raise Refusal("caduceus-nas-schedule-field-invalid")
 f=v.get("frequency","daily"); t=v.get("time",v.get("at","02:00")); w=v.get("weekday",v.get("day","Mon"))
 if f not in {"daily","weekly"}: raise Refusal("caduceus-nas-schedule-frequency-invalid")
 if not isinstance(t,str) or len(t)!=5 or t[2]!=":" or not t[:2].isdigit() or not t[3:].isdigit() or int(t[:2])>23 or int(t[3:])>59: raise Refusal("caduceus-nas-schedule-time-invalid")
 if f=="weekly" and w not in {"Mon","Tue","Wed","Thu","Fri","Sat","Sun"}: raise Refusal("caduceus-nas-schedule-weekday-invalid")
...
def units(s, enabled=True):
 ...
 marker="# caduceus-nas-sync-schedule: "+json.dumps(record,sort_keys=True,separators=(",",":"))
 return ("[Unit]\nDescription=Homeserver safe NAS sync\n\n[Service]\nType=oneshot\nExecStart=/usr/bin/bash /usr/local/sbin/safe-nas-sync.sh\n",f"{marker}\n[Unit]\nDescription=Homeserver safe NAS sync timer\n\n[Timer]\nOnCalendar={cal}\nPersistent=true\nUnit={SERVICE}\n\n[Install]\nWantedBy=timers.target\n")
...
def read_timer_schedule(text):
 for line in text.splitlines()[:1]:
  if line.startswith("# caduceus-nas-sync-schedule: "):
   value=json.loads(line.split(": ",1)[1]); base=schedule(value)
   enabled=value.get("enabled",True)
   if not isinstance(enabled,bool): raise Refusal("caduceus-nas-schedule-enabled-invalid")
...
def schedule_update(p,planned):
 n=p.get("schedule")
 if not isinstance(n,dict): raise Refusal("caduceus-nas-schedule-invalid")
 ...
 if enabled:
  ...
  for c in cmds:
   if subprocess.run(c,check=False).returncode: raise Refusal("caduceus-nas-schedule-update-refused")
 else:
  if subprocess.run(cmds[0],check=False).returncode: raise Refusal("caduceus-nas-schedule-update-refused")
  if subprocess.run(cmds[1],check=False).returncode:
   raise Refusal("caduceus-nas-schedule-update-refused")
  if subprocess.run(cmds[2],check=False).returncode: raise Refusal("caduceus-nas-schedule-update-refused")
...
def job_status(p,planned):
 ...
 if not valid(h): raise Refusal("caduceus-nas-job-handle-invalid")
 if planned: return receipt("sync-job-status",True,[],jobHandle=h,status="planned",progress=None)
 rec=root()/f"{h}.json"
 if not rec.is_file(): raise Refusal("caduceus-nas-job-not-found")
...
def worker(h):
 ...
 try:
  ...
  return 1
def dispatch(x):
 if set(x)-{"actuator","metadata"} or x.get("actuator")!="nas-sync" or not isinstance(x.get("metadata"),dict): raise Refusal("caduceus-nas-request-invalid")
 p=x["metadata"]; a=p.get("action"); planned=p.get("dryRun",p.get("planned",False))
 if not isinstance(planned,bool): raise Refusal("caduceus-nas-planned-invalid")
 ...
 raise Refusal("caduceus-nas-action-invalid")
def main():
 ...
 except (UnicodeDecodeError,json.JSONDecodeError,Refusal,OSError,subprocess.SubprocessError) as e: value=fail(str(e) if isinstance(e,Refusal) else "caduceus-nas-request-invalid")
